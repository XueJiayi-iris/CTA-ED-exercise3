...
eumat <- as.matrix(euclidean_dist) #convert to a matrix

#generate data frame keeping only the row for Theresa May
eumatdf <- as.data.frame(eumat[23, c(1:22, 24)])

colnames(eumatdf) <- "euclidean_may" #rename column
  
#create column variable from rownames
eumatdf <- tibble::rownames_to_column(eumatdf, "username")

ggplot(eumatdf) +
  geom_point(aes(x=reorder(username, -euclidean_may), y= euclidean_may)) + 
  coord_flip() +
  xlab("MP username") +
...
mhtmat <- as.matrix(manhattan_dist) 
mhtmatdf <- as.data.frame(mhtmat[23, c(1:22, 24)])

colnames(mhtmatdf) <- "manhattan_may" 
mhtmatdf <- tibble::rownames_to_column(mhtmatdf, "username")

ggplot(mhtmatdf) +
  geom_point(aes(x=reorder(username, -manhattan_may), y= manhattan_may)) + 
  coord_flip() +
  xlab("MP username") +
...
sum_corpus <- speeches %>%
  group_by(speaker) %>%
  summarise(mean_Flesh = mean(flesch, na.rm=TRUE),
            mean_FOG = mean(fog, na.rm=TRUE),
            mean_SMOG = mean(smog, na.rm=TRUE),
            SD_Flesh=sd(flesch, na.rm=TRUE),
            SD_FOG=sd(fog, na.rm=TRUE),
            SD_SMOG=sd(smog, na.rm=TRUE),
            N=length(speaker))

sum_corpus
```
All three complexity measures consider syllable counts as one of the criteria for identifying "complex words", and the results show the same ranking of speakers in terms of the readability of their speeches. D. Cameron's speeches are the most readable, while J.L.R. Zapatero's are the least readable.
